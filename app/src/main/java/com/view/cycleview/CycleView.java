...
public class CycleView extends FrameLayout implements ViewPager.OnPageChangeListener{
    ...
    // 指示器图片，被选择状态
    private int mIndicatorSelected = R.mipmap.point_blue_lighted;
    // 指示器图片，未被选择状态
    private int mIndicatorUnselected = R.mipmap.point_blue_normal;
    // 需要轮播的View集合
    ...
    // 轮播view默认占位图
    private Drawable defaultImage = getResources().getDrawable(R.mipmap.icon_product_defoult);
    ...
    public void setData(List<CycleModel> mData, int defaultPosition, Context context,
        ...
        if (isCycle) {
            // 添加轮播图View，数量为集合数+2
            // 将最后一个View添加进来
            mViews.add(getCycleView(context, mData.get(size - 1).getPicURL(), defaultImage));
            for (int i = 0; i < size; i++) {
                mViews.add(getCycleView(context, mData.get(i).getPicURL(), defaultImage));
            }
            // 将第一个View添加进来
            mViews.add(getCycleView(context , mData.get(0).getPicURL(), defaultImage));
        } else {
            // 只添加对应数量的View
            for (int i = 0; i < size; i++) {
                mViews.add(getCycleView(context, mData.get(i).getPicURL(), defaultImage));
    ...
    private View getCycleView(Context context, String url, Drawable defaultImage) {
        ...
        // 加载图片资源
        ImageLoader.getInstance().loadImg(context,imageView, url);
